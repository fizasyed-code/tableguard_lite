...
REVISION = '27-recovery-delta-feasible-rollout-v1'
...
def find_completed_training(root):
    """Use the latest successful Notebook 26 recovery-augmented delta checkpoint."""
    root=Path(root).resolve()
    parent=root/'artifacts'/'smolvla_train_26_recovery'
    if not parent.is_dir():
        return None,{'status':'waiting_for_training_26',
                     'reason':'No Notebook 26 recovery training folder found.'}
    ...
    if run is None:
        return None,{'status':'waiting_for_training_26',
                     'reason':'No completed reload-verified Notebook 26 run found.'}
    checkpoint=(run/'checkpoint_final').resolve()
    if Path(r.get('checkpoint_path','')).resolve()!=checkpoint:
        raise ValueError('Notebook 26 checkpoint path mismatch.')
    ...
    if c.get('action_representation')!='delta joint position targets relative to current pre-action observation state':
        raise ValueError('Notebook 26 checkpoint is not the expected delta-action representation.')
    if not c.get('recovery_augmentation'):
        raise ValueError('Notebook 26 recovery metadata missing from contract.')
    ...
    if int(ts.get('total_updates',-1))!=int(r.get('checkpoint_total_optimizer_updates',-2)):
        raise ValueError('Notebook 26 total update count mismatch.')
