...
def show_result(run):
    ...
    lines=['TABLEGUARD 27 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_27'
    ...
    if waiting:
        text='TABLEGUARD 27 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_27';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 27 LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 26 recovery-augmented delta weights + feasible delta projection | backend:',backend,flush=True)
