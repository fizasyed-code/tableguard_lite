...
def validate_contract(c, stats):
    ...
    if not isinstance(c.get('episodes'),int) or c.get('episodes') < 1 or c.get('action_dim')!=12 or c.get('state_dim')!=12:
        raise ValueError('Expected a validated multi-episode 12-axis diagnostic')
    ...
    if c.get('action_units')!='radians' or c.get('action_representation')!='delta joint position targets relative to current pre-action observation state':
        raise ValueError('Unexpected delta-action semantics')
