...
def show_result(run):
    ...
    lines=['TABLEGUARD 12D — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_12d'
    ...
    if waiting:
        text='TABLEGUARD 12D — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_12d';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 12D LEARNED ROLLOUT:',run,flush=True)
