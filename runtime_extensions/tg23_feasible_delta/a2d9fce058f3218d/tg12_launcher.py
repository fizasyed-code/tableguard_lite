...
def show_result(run):
    ...
    lines=['TABLEGUARD 23 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_23'
    ...
    if waiting:
        text='TABLEGUARD 23 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_23';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 23 LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 20 delta-action weights + feasible delta projection | backend:',backend,flush=True)
