...
def show_result(run):
    ...
    lines=['TABLEGUARD 32 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_32'
    ...
    if waiting:
        text='TABLEGUARD 32 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    # Notebook 20 is trained from the validated Notebook 14 multi-episode batch.
    # The rollout scene remains the original passed 07C scene, but the checkpoint
    # contract no longer needs to claim a single source_run.
    if contract.get('action_representation')!='delta joint position targets relative to current pre-action observation state':
        raise ValueError('Expected Notebook 20 delta-action contract')
    if int(contract.get('episodes',0)) < 5:
        raise ValueError('Expected at least five successful training episodes')
    parent=root/'artifacts'/'learned_rollout_32';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 32 LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 31 17-demo-envelope package of Notebook 29 weights + feasible delta projection | backend:',backend,flush=True)
