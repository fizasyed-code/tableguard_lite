...
EXEC_STEPS=20  # match training cadence: execute 100 ms, then observe/replan
...
class Simulator:
    def __init__(self,cfg):
        ...
        self.report=dict(revision=cfg['revision'],status='initializing',error=None,stops=[],
            task_scope='17-demo-envelope delta-action right-arm cup-transfer diagnostic with unchanged 5201-update weights, state-feasible projection, 5-step gripper horizon hold, and fixed inactive-left-arm hold; NOT complete bimanual project',
            ...
            source_training_episodes=int(read_json(Path(cfg['checkpoint'])/'contract.json').get('episodes',0)),checkpoint_total_updates=cfg['checkpoint_total_updates'],
            ...
            privileged_state_used_to_steer_policy=False,inactive_left_arm_mask=True,delta_training_envelope_projection=True,delta_current_state_feasibility_projection=True,control_cadence_matches_training=True,simulator_paused_during_inference=True,
            continuous_wall_clock_realtime=False,action_dt_s=DT,action_chunk_steps=CHUNK,executed_steps_per_policy_call=EXEC_STEPS,receding_horizon_control=True,control_replan_interval_s=DT*EXEC_STEPS,
    ...
    def step_loop(self):
        ...
        maxchunks=int(math.ceil(self.cfg['max_sim_s']/(DT*EXEC_STEPS)))
        ...
        with (self.out/'rollout_observations.jsonl').open('x',encoding='utf-8') as observations:
            for frame in range(maxchunks):
                ...
                record=dict(frame_id=frame,step=self.steps,time_s=float(self.data.time),images=paths,
                    ...
                    action_order=self.order,policy_source='notebook 11 checkpoint; fresh closed-loop inference',
                ...
                with np.load(rp,allow_pickle=False) as z:delta=np.array(z['requested_deltas_rad'],copy=True)
                if delta.shape!=(20,12) or not np.isfinite(delta).all():raise ValueError('Invalid delta policy chunk')
                # Start from the learned delta chunk.
                masked_delta=delta.copy()
                left_idx=[i for i,n in enumerate(self.order) if n.startswith('left_')]
                right_idx=[i for i,n in enumerate(self.order) if n.startswith('right_')]
                if len(left_idx)!=6 or len(right_idx)!=6:
                    raise ValueError('Expected six joints per arm')

                # Inactive left arm: no learned motion for this cup-only diagnostic.
                masked_delta[:,left_idx]=0.0

                # Active right arm: project each predicted delta into the empirical
                # per-joint delta envelope observed in the successful Notebook 20
                # training demonstrations. This uses only training-data statistics;
                # no object pose, phase, or IK enters the projection.
                delta_lo=np.asarray(self.delta_stats['minimum'],dtype=np.float64)
                delta_hi=np.asarray(self.delta_stats['maximum'],dtype=np.float64)
                if delta_lo.shape!=(12,) or delta_hi.shape!=(12,) or np.any(delta_lo>delta_hi):
                    raise ValueError('Invalid delta training envelope')

                envelope_before=masked_delta.copy()

                # Per-request feasible delta bounds derived only from the current
                # measured joint state and the validated actuator/joint limits.
                joint_lo=np.asarray([r['low'] for r in self.rows_map],dtype=np.float64)
                joint_hi=np.asarray([r['high'] for r in self.rows_map],dtype=np.float64)
                feasible_lo=joint_lo-st
                feasible_hi=joint_hi-st

                # Active right arm must satisfy BOTH:
                #   1) empirical delta range seen in successful training data
                #   2) current-state joint feasibility
                right_lo=np.maximum(delta_lo[right_idx],feasible_lo[right_idx])
                right_hi=np.minimum(delta_hi[right_idx],feasible_hi[right_idx])
                if np.any(right_lo>right_hi):
                    raise GuardStop('No feasible right-arm delta interval at current state')

                masked_delta[:,right_idx]=np.minimum(
                    np.maximum(masked_delta[:,right_idx],right_lo[None,:]),
                    right_hi[None,:])

                # Notebook 33 showed that the immediate right-arm action is accurate,
                # while gripper error grows across the 20-step open-loop chunk.
                # Keep the normal 100 ms camera/state replanning cadence, but trust
                # only the first 5 predicted gripper steps (25 ms) and hold the
                # fifth gripper delta for the remaining 15 steps.
                gripper_idx=next((i for i,n in enumerate(self.order) if n=='right_gripper'),None)
                if gripper_idx is None:
                    raise ValueError('right_gripper not found in action order')
                gripper_before_horizon_hold=masked_delta[:,gripper_idx].copy()
                GRIPPER_TRUST_STEPS=5
                if masked_delta.shape[0] < GRIPPER_TRUST_STEPS:
                    raise ValueError('Action chunk shorter than gripper trust horizon')
                masked_delta[GRIPPER_TRUST_STEPS:,gripper_idx]=masked_delta[GRIPPER_TRUST_STEPS-1,gripper_idx]

                # Active right arm: state-relative delta action.
                raw=st[None,:]+masked_delta

                # Inactive left arm: fixed reset targets from the validated scene mapping.
                reset=np.asarray(self.initial_q,dtype=np.float64)
                raw[:,left_idx]=reset[left_idx][None,:]

                if not np.isfinite(raw).all():raise ValueError('Nonfinite reconstructed absolute targets')
                if np.any(raw[:,right_idx] < joint_lo[right_idx][None,:]-1e-10) or np.any(raw[:,right_idx] > joint_hi[right_idx][None,:]+1e-10):
                    raise RuntimeError('Feasible-delta projection failed to keep right-arm targets within joint limits')
                self.policy_calls+=1
                record.update(inference_completed=True,inference_s=response['inference_s'],noise_seed=response['seed'],
                              raw_delta_chunk=delta.tolist(),pre_envelope_delta_chunk=envelope_before.tolist(),masked_delta_chunk=masked_delta.tolist(),gripper_delta_before_horizon_hold=gripper_before_horizon_hold.tolist(),gripper_trust_steps=GRIPPER_TRUST_STEPS,gripper_horizon_hold=True,inactive_left_arm_mask=True,inactive_left_arm_hold='initial_settled_joint_positions',delta_training_envelope_projection=True,delta_current_state_feasibility_projection=True,feasible_delta_lower_rad=feasible_lo.tolist(),feasible_delta_upper_rad=feasible_hi.tolist(),raw_action_chunk=raw.tolist(),action_chunk_dt_s=DT,predicted_chunk_steps=CHUNK,executed_chunk_steps=EXEC_STEPS,applied_action_chunk=[])
                try:
