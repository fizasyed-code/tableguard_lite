...
def show_result(run):
    ...
    lines=['TABLEGUARD 34 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_34'
    ...
    if waiting:
        text='TABLEGUARD 34 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_34';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 34 LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 31/29 unchanged 5201-update weights + 5-step gripper horizon hold | backend:',backend,flush=True)
