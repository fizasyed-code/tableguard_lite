...
REVISION = '34-gripper-short-horizon-hold-v1'
...
def find_completed_training(root):
    """Use latest reload-verified Notebook 31 derived envelope checkpoint."""
    root=Path(root).resolve()
    parent=root/'artifacts'/'smolvla_checkpoint_31_envelope'
    if not parent.is_dir():
        return None,{'status':'waiting_for_checkpoint_31','reason':'Run Notebook 31 first.'}
    ...
    if run is None:
        return None,{'status':'waiting_for_checkpoint_31',
                     'reason':'No completed reload-verified Notebook 31 checkpoint.'}
    checkpoint=(run/'checkpoint_final').resolve()
    c=read_json(checkpoint/'contract.json')
    if int(c.get('runtime_delta_envelope_episodes',0))!=17:
        raise ValueError('Expected 17-demo runtime delta envelope.')
