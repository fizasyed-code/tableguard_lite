...
REVISION = '30-graspdiv-delta-feasible-rollout-v1'
...
def find_completed_training(root):
    root=Path(root).resolve(); parent=root/'artifacts'/'smolvla_train_29_graspdiv'
    if not parent.is_dir(): return None,{'status':'waiting_for_training_29','reason':'Run Notebook 29 first.'}
    runs=sorted([p for p in parent.iterdir() if p.is_dir() and (p/'training_report.json').is_file()],key=lambda p:p.stat().st_mtime,reverse=True)
    run=None;r=None
    for candidate in runs:
        rr=read_json(candidate/'training_report.json')
        if rr.get('status')=='training_run_completed' and rr.get('checkpoint_reloaded') is True: run=candidate;r=rr;break
    if run is None: return None,{'status':'waiting_for_training_29','reason':'No completed reload-verified Notebook 29 run found.'}
    checkpoint=(run/'checkpoint_final').resolve()
    if Path(r.get('checkpoint_path','')).resolve()!=checkpoint: raise ValueError('Notebook 29 checkpoint path mismatch.')
    c=read_json(checkpoint/'contract.json')
    if c.get('action_representation')!='delta joint position targets relative to current pre-action observation state': raise ValueError('Notebook 29 checkpoint is not delta-action.')
    ...
    if int(ts.get('total_updates',-1))!=int(r.get('checkpoint_total_optimizer_updates',-2)): raise ValueError('Notebook 29 total update count mismatch.')
    return {'training_run':str(run),'checkpoint':str(checkpoint),'training_report':r,'training_report_sha256':sha(run/'training_report.json'),'expected_hashes_file':None,'reload_verification':None},None
