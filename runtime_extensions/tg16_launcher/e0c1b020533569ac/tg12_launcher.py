...
def show_result(run):
    ...
    lines=['TABLEGUARD 16 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_16'
    ...
    if waiting:
        text='TABLEGUARD 16 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_16';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 16 LEARNED ROLLOUT:',run,flush=True)
