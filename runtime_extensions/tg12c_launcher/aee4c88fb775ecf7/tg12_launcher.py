...
def show_result(run):
    ...
    lines=['TABLEGUARD 12C — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_12c'
    ...
    if waiting:
        text='TABLEGUARD 12C — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_12c';parent.mkdir(exist_ok=True)
