...
def show_result(run):
    ...
    lines=['TABLEGUARD 17B — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_17b'
    ...
    if waiting:
        text='TABLEGUARD 17B — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_17b';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 17B LEARNED ROLLOUT:',run,flush=True)
