...
def show_result(run):
    ...
    lines=['TABLEGUARD 21B — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_21b'
    ...
    if waiting:
        text='TABLEGUARD 21B — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_21b';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 21B LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 20 delta-action weights + inactive-left-arm mask | backend:',backend,flush=True)
