...
class Simulator:
    def __init__(self,cfg):
        ...
        self.report=dict(revision=cfg['revision'],status='initializing',error=None,stops=[],
            task_scope='delta-action right-arm cup-transfer diagnostic with disclosed inactive-left-arm mask; NOT complete bimanual project',
    ...
    def step_loop(self):
        ...
        with (self.out/'rollout_observations.jsonl').open('x',encoding='utf-8') as observations:
            for frame in range(maxchunks):
                ...
                # Cup-only task: freeze the six inactive LEFT-arm deltas.
                ...
                # Reconstruct absolute targets from the same fresh pre-action state.
                ...
                record.update(inference_completed=True,inference_s=response['inference_s'],noise_seed=response['seed'],
                              raw_delta_chunk=delta.tolist(),masked_delta_chunk=masked_delta.tolist(),inactive_left_arm_mask=True,raw_action_chunk=raw.tolist(),action_chunk_dt_s=DT,predicted_chunk_steps=CHUNK,executed_chunk_steps=EXEC_STEPS,applied_action_chunk=[])
                try:
