...
class Simulator:
    def __init__(self,cfg):
        ...
        self.report=dict(revision=cfg['revision'],status='initializing',error=None,stops=[],
            task_scope='delta-action right-arm cup-transfer diagnostic with training-envelope projection and fixed inactive-left-arm hold; NOT complete bimanual project',
            ...
            privileged_state_used_to_steer_policy=False,inactive_left_arm_mask=True,delta_training_envelope_projection=True,control_cadence_matches_training=True,simulator_paused_during_inference=True,
    ...
    def step_loop(self):
        ...
        with (self.out/'rollout_observations.jsonl').open('x',encoding='utf-8') as observations:
            for frame in range(maxchunks):
                ...
                masked_delta[:,right_idx]=np.clip(
                    masked_delta[:,right_idx],
                    delta_lo[right_idx][None,:],
                    delta_hi[right_idx][None,:])
