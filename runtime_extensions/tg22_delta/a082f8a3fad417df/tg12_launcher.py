...
def show_result(run):
    ...
    lines=['TABLEGUARD 22 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_22'
    ...
    if waiting:
        text='TABLEGUARD 22 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_22';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 22 LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 20 delta-action weights + training-envelope projection | backend:',backend,flush=True)
