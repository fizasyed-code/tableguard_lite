...
from tg12_common import (REVISION,SOURCE_PILOT,SOURCE_CUP_RUN,Lease,read_json,write_json,sha,
                        find_completed_training,safe_name)
...
def show_result(run):
    ...
    lines=['TABLEGUARD 12A — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_12a'
    ...
    if waiting:
        text='TABLEGUARD 12A — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_12a';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        settings=dict(revision=REVISION,root=str(root),output=str(run),
            ...
            expected_hashes_file=training['expected_hashes_file'],training_versions=sr.get('versions',{}),
        ...
        print('NEW 12A LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: saved notebook11 weights with required reload verification | backend:',backend,flush=True)
