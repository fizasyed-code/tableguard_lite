...
def show_result(run):
    ...
    lines=['TABLEGUARD 12 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_12'
    ...
    if waiting:
        text='TABLEGUARD 12 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_12';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 12 LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: completed notebook11 final weights | backend:',backend,flush=True)
