...
def main(settings_file):
    ...
    try:
        ...
        if protected!={k:v for k,v in expected.items() if Path(k).name!='optimizer_state.pt'}:
            raise RuntimeError('Checkpoint inference files differ from notebook 11 saved hashes')
        ...
        event(out,'policy','load_notebook11_checkpoint_strictly',device=device)
        policy=SmolVLAPolicy.from_pretrained(checkpoint,config=mc,strict=True,local_files_only=True).float().to(device).eval()
