...
def show_result(run):
    ...
    lines=['TABLEGUARD 21A — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_21a'
    ...
    if waiting:
        text='TABLEGUARD 21A — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_21a';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 21A LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 20 delta-action weights (5 successful episodes) | backend:',backend,flush=True)
