...
class Simulator:
    def __init__(self,cfg):
        ...
        self.report=dict(revision=cfg['revision'],status='initializing',error=None,stops=[],
            task_scope='delta-action learned cup-transfer integration diagnostic; NOT complete bimanual project',
    ...
    def step_loop(self):
        ...
        with (self.out/'rollout_observations.jsonl').open('x',encoding='utf-8') as observations:
            for frame in range(maxchunks):
                ...
                # Reconstruct absolute joint-position targets from the same fresh
                # pre-action state that was sent to the VLA for this request.
                raw=st[None,:]+delta
                ...
                record.update(inference_completed=True,inference_s=response['inference_s'],noise_seed=response['seed'],
                              raw_delta_chunk=delta.tolist(),raw_action_chunk=raw.tolist(),action_chunk_dt_s=DT,predicted_chunk_steps=CHUNK,executed_chunk_steps=EXEC_STEPS,applied_action_chunk=[])
                try:
