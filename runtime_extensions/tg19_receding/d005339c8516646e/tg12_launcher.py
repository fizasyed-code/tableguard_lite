...
def show_result(run):
    ...
    lines=['TABLEGUARD 19 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_19'
    ...
    if waiting:
        text='TABLEGUARD 19 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_19';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 19 LEARNED ROLLOUT:',run,flush=True)
