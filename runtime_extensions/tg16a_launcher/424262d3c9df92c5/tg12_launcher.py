...
def show_result(run):
    ...
    lines=['TABLEGUARD 16A — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_16a'
    ...
    if waiting:
        text='TABLEGUARD 16A — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_16a';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 16A LEARNED ROLLOUT:',run,flush=True)
