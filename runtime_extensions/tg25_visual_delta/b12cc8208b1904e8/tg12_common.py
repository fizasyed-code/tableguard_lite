...
REVISION = '25-visual-adapted-delta-feasible-rollout-v1'
...
def find_completed_training(root):
    """Use the latest successful Notebook 24B visual-adapted delta checkpoint."""
    root=Path(root).resolve()
    parent=root/'artifacts'/'smolvla_train_24b_visual'
    if not parent.is_dir():
        return None,{'status':'waiting_for_training_24b',
                     'reason':'No Notebook 24B visual training folder found.'}
    ...
    if run is None:
        return None,{'status':'waiting_for_training_24b',
                     'reason':'No completed reload-verified Notebook 24B run found.'}
    checkpoint=(run/'checkpoint_final').resolve()
    if Path(r.get('checkpoint_path','')).resolve()!=checkpoint:
        raise ValueError('Notebook 24B checkpoint path mismatch.')
    ...
    if c.get('action_representation')!='delta joint position targets relative to current pre-action observation state':
        raise ValueError('Notebook 24B checkpoint is not the expected delta-action representation.')
    ...
    if int(ts.get('total_updates',-1))!=int(r.get('checkpoint_total_optimizer_updates',-2)):
        raise ValueError('Notebook 24B total update count mismatch.')
