...
def show_result(run):
    ...
    lines=['TABLEGUARD 25 — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_25'
    ...
    if waiting:
        text='TABLEGUARD 25 — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_25';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 25 LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 24B visual-adapted delta weights + feasible delta projection | backend:',backend,flush=True)
