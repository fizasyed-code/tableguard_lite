...
EXEC_STEPS=10  # receding-horizon: execute 50 ms, then observe/replan
...
class Simulator:
    def __init__(self,cfg):
        ...
        self.report=dict(revision=cfg['revision'],status='initializing',error=None,stops=[],
            task_scope='delta-action right-arm cup-transfer diagnostic with fixed inactive-left-arm reset hold; NOT complete bimanual project',
            ...
            privileged_state_used_to_steer_policy=False,inactive_left_arm_mask=True,simulator_paused_during_inference=True,
    ...
    def step_loop(self):
        ...
        with (self.out/'rollout_observations.jsonl').open('x',encoding='utf-8') as observations:
            for frame in range(maxchunks):
                ...
                # Cup-only task: hold the six inactive LEFT-arm joints at the
                # original settled reset positions. Zero delta relative to the current
                # state would ratchet physical drift forward, so do not use current
                # left-arm state as the inactive target.
                masked_delta=delta.copy()
                left_idx=[i for i,n in enumerate(self.order) if n.startswith('left_')]
                if len(left_idx)!=6:raise ValueError('Expected six inactive left-arm joints')
                ...
                record.update(inference_completed=True,inference_s=response['inference_s'],noise_seed=response['seed'],
                              raw_delta_chunk=delta.tolist(),masked_delta_chunk=masked_delta.tolist(),inactive_left_arm_mask=True,inactive_left_arm_hold='initial_settled_joint_positions',raw_action_chunk=raw.tolist(),action_chunk_dt_s=DT,predicted_chunk_steps=CHUNK,executed_chunk_steps=EXEC_STEPS,applied_action_chunk=[])
                try:
