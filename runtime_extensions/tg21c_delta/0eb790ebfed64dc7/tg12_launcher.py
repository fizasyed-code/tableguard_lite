...
def show_result(run):
    ...
    lines=['TABLEGUARD 21C — FINAL SUMMARY','Run: '+str(run)]
...
def run_12(root,sources,backend='pytorch_cuda',start_new=False):
    ...
    existing_parent=root/'artifacts'/'learned_rollout_21c'
    ...
    if waiting:
        text='TABLEGUARD 21C — PREREQUISITE STATUS\n'+json.dumps(waiting,indent=2)
    ...
    parent=root/'artifacts'/'learned_rollout_21c';parent.mkdir(exist_ok=True)
    ...
    with ExitStack() as locks:
        ...
        print('NEW 21C LEARNED ROLLOUT:',run,flush=True)
        print('Checkpoint: Notebook 20 delta-action weights + fixed left-arm reset hold | backend:',backend,flush=True)
